...
export const parseFile = async (file: string, to?: string) => {
  ...
  const mediaExtensions = ['.mp4', '.avi', '.mov', '.mkv', '.flv', '.rmvb', '.mp3', '.wav', '.flac', '.aac', '.ogg'];
  ...
  data.md5 = data.md5 || await getFileMd5(file)
...
const parseMp3Mp4 = async file => {
  ...
  console.log('typeof tags.title :>>', typeof tags.title)
  return {
    duration,
    size,
    title: tags?.title,
...
parseMp3Mp4('I:/影视/音乐/音乐1/mp3/[张明敏]龙的传人-张明敏.mp3')
.then(o=>{
  console.log('o :>>', o)
})
